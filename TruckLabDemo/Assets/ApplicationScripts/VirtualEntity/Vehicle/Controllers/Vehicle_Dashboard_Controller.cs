...
namespace ApplicationScripts.VirtualEntity.Vehicle.Controllers
{
    ...
    public class Vehicle_Dashboard_Controller : Base_UI_Controller
    {
        ...
        {
            TopBar = UiInstance.transform.Find("Top Bar")?.gameObject;
        ...
        private void UpdatePage1()
        {
            ...
            // Retrieve the current active input strategy name
            string currentStrategyName = VeVehicle.KinematicsController.GetCurrentInputStrategyName();
            if (ActuationSourceDropdown != null && ActuationSourceDropdown.options.Count > 0)
            {
                // Find the index of the dropdown option that matches the current active strategy
                int strategyIndex = ActuationSourceDropdown.options.FindIndex(option => option.text == currentStrategyName);

                // If found and different from current selection, update the dropdown
                if (strategyIndex >= 0 && ActuationSourceDropdown.value != strategyIndex)
                {
                    ActuationSourceDropdown.value = strategyIndex;
                }
            }
        ...
        public void OnToggleTractorTrail(bool status)
        {
            if (VeVehicle?.AnimationController?.TractorTrajectory != null)
            {
                ...
                TractorTrailToggle.isOn = status;
            }
        }
        ...
        public void OnToggleDashboardPin(bool status)
        {
