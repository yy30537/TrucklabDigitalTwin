...
using UnityEngine;
using VehiclePhysics.Scripts.Vehicle.Misc;

namespace VehiclePhysics.Scripts.Vehicle.Input
{
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (inputType == InputType.Standard)
                    {
                        ...
                        {
                            vertical = (Mathf.Clamp01(UnityEngine.Input.GetAxisRaw("Vertical")) - 0.5f) * 2f;
                        }
                        else if (verticalInputType == VerticalInputType.Composite)
                        {
                            float accelerator = Mathf.Clamp01(UnityEngine.Input.GetAxisRaw("Accelerator"));
                            float brake = Mathf.Clamp01(UnityEngine.Input.GetAxisRaw("Brake"));
                ...
                try
                {
                    vehicleController.input.Handbrake = UnityEngine.Input.GetAxis("Handbrake");
                }
                catch
                {
                    Debug.LogWarning("Handbrake axis not set up, falling back to default (VeSpace).");
                    vehicleController.input.Handbrake = UnityEngine.Input.GetKey(KeyCode.Space) ? 1f : 0f;
                }
                ...
                if (!vehicleController.transmission.automaticClutch)
                {
                    try
                    {
                        vehicleController.input.Clutch = UnityEngine.Input.GetAxis("Clutch");
                    }
                    catch
                    {
                ...
                if (vehicleController.flipOver.manual)
                {
                    try
                    {
                        ...
                        if (UnityEngine.Input.GetButtonDown("FlipOver") && vehicleController.flipOver.flippedOver)
