...
using UnityEngine;
using VehiclePhysics.GUI.Scripts;
...
namespace VehiclePhysics.Scripts.Vehicle 
{
    ...
    public class Transmission
    {
        ...
        /// Automatic - automatic gear changing. Allows for gear skipping (e.g. 3rd->5th) which can be useful in trucks and other high gear count VehicleConfigs.
        ...
        [Tooltip("Behavior when switching from neutral or forward gears to reverse gear. " +
            "Auto - if the vehicleTransform speed is low enough and vertical input negative, transmission will shift to reverse. " + 
            ...
            " otherwise the vehicleTransform will stand still in neutral with brakes applied.")]
        ...
        /// <summary>
        /// Describes how much clutch will 'grab' as the pedal is released. When the currentTractorPosY of the curve is at 1 this means that the clutch is fully engaged, i.e. there 
        /// is no slip between transmission and engine. When the currentTractorPosY of the curve is at 0 there is no connection between engine and transmission / wheels.
        /// 0 on the currentTractorPosX axis represents fully released clutch pedal while 1 represents fully pressed pedal. Normally every clutch would have two points,
        /// [0,1] and [1,0], and the in-between will vary from vehicleTransform to vehicleTransform.
        /// </summary>
        [Tooltip("Describes how much clutch will 'grab' as the pedal is released. When the currentTractorPosY of the curve is at 1 this means that the clutch is fully engaged, i.e. there " +
            "is no slip between transmission and engine. When the currentTractorPosY of the curve is at 0 there is no connection between engine and transmission / wheels." +
            "0 on the currentTractorPosX axis represents fully released clutch pedal while 1 represents fully pressed pedal. Normally every clutch would have two points," +
            "[0,1] and [1,0], and the in-between will vary from vehicleTransform to vehicleTransform.")]
        ...
        public void TorqueSplit(float torque, float topRPM)
        {
            ...
            // Non-tracked vehicleTransform torque split
            ...
            // Tracked vehicleTransform torque split
            else
            {
                int leftCount = 0;
                int rightCount = 0;
        ...
        private void AutomaticShift()
        {
            ...
            else
            {
                if (vc.ForwardVelocity > 0.2f)
                {
					...
					{
                        // Check if downshift allowed on tracked vehicleTransform
                        if (vc.tracks.trackedVehicle)
                        {
