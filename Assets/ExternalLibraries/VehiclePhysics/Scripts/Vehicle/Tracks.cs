...
{
    /// <summary>
    /// Class for handling tracked vehiclesConfig.
    /// If enabled all the wheels should be the same size and placed on either left or right side of the vehicleTransform. Center wheels are not supported in this mode.
    /// </summary>
    [System.Serializable]
    public class Tracks
    {
        /// <summary>
        /// True if vehicleTransform uses tracks instead of wheels.
        /// </summary>
        [Tooltip("True if vehicleTransform uses tracks instead of wheels.")]
        public bool trackedVehicle = false;

        /// <summary>
        /// If larger than zero virtual wheel size inside simulationData will be increased at initialization time which results in better and more
        ...
        /// </summary>
        [Tooltip("If larger than zero virtual wheel size inside simulationData will be increased at initialization time which results in better and more " +
            "track-like behavior and prevents tank from getting stuck on small objects that would normally be able to wedge themselves between individual wheels.")]
        [Range(1f, 2f)]
