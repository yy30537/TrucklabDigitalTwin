        ...
        public static void CheckForUrdfCompatibility(Transform transform, GeometryTypes type)
        {
            ...
            if (IsTransformed(childTransform, type))
            {
                Debug.LogWarning("Changes to the transform of " + childTransform.name + " cannot be exported to URDF. " +
                                 "Make any translation, rotation, or scale changes to the parent Visual or CollisionController object instead.",
                    childTransform);
            }

            if (!transform.HasExactlyOneChild())
                Debug.LogWarning("Only one Geometry element is allowed for each Visual or CollisionController element. In "
                                 + transform.parent.parent.name + ", move each Geometry into its own Visual or CollisionController.", transform);
        }
